...
public class MagicGenerater implements IWorldGenerator 
{
	...
	public void generateSurface(World world, Random random, int blockX, int blockZ)
	{
		...
		(new WorldGenMEOres(MagicalExperience.expore.blockID, 0, 10)).generate(world, random, Xcoord, Ycoord, Zcoord);
		
		int Xcoord1 = blockX + random.nextInt(2);
		int Ycoord1 = random.nextInt(60);
		int Zcoord1 = blockZ + random.nextInt(3);
		(new WorldGenMEOres(MagicalExperience.expore.blockID, 1,  15)).generate(world, random, Xcoord1, Ycoord1, Zcoord1);	
	}
}
