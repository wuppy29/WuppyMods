...
public class UpdateChecker
{
	...
	{
		int currentVersion = ImprovedMobs.VERSION;
		int nextVersion = getNewest();
		
		if (currentVersion < nextVersion)
		{
			ImprovedMobs.updates = getUpdate(nextVersion);
			ImprovedMobs.outdated = true;
		}
		else
		{
			ImprovedMobs.outdated = false;
		}
	}
	
	public static int getNewest() throws IOException
	{
		URL url = new URL("http://wuppy29.com/minecraft/mods/im/newest.txt");
	...
	{
		URL url = new URL("http://wuppy29.com/minecraft/mods/im/" + version + ".txt");
		HttpURLConnection connection = (HttpURLConnection) url.openConnection();
		connection.setRequestMethod("POST");
		connection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
		connection.setRequestProperty("Content-Language", "en-US");
